...
async def update_item(item: Item, session: AsyncSession = Depends(getSession)):
    """updates one item"""
    try:
        return await AdminCrud.update_item(item=item, session=session)
...
async def delete_item(barcode: str, session: AsyncSession = Depends(getSession)):
    """Deletes Item from database using barcode"""
    try:
        await AdminCrud.delete_item(barcode=barcode, session=session)
        return
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e)) from e
...
async def add_item(item: ItemDetails, session: AsyncSession = Depends(getSession)):
    """adds one item"""
    try:
        result = await AdminCrud.create_item(item=item, session=session)
        return result
    except Exception as e:
